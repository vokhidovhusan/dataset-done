...
def main(args):
    ...
    for sub in args.sub_dir:
        ...
        print('{}/{}.txt'.format(path, args.operation[0]))
        ...
        for line in lines:
            ...
            if os.path.exists(line[:-1]):
                ...
                pp = Path(os.path.join(path, 'labels_{}'.format(args.operation[0])))
                ppp = Path(os.path.join(path, 'labels'.format(args.operation[0])))                
                pp.mkdir(exist_ok=True)
                file_name = line.rsplit('/', 1)[-1][:-1]   
                print(file_name)             
                if(utils.copy_file_to_directories_full_path(line[:-1], '{}/{}'.format(p, file_name))):
                    count += 1
                if(utils.copy_file_to_directories_full_path('{}/{}txt'.format(ppp, file_name[:-3]), '{}/{}txt'.format(pp, file_name[:-3]))):
                    count_l += 1
                # else:
